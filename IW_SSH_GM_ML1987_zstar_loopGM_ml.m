...
%     PcapWKB_jm(:,jm)=Pcap0/P0_fac;
    ...
    for momg=1:nomg
        ...
        %  z=-zstar
        %   with Icap=1
        Acap(1,1)=1/dz-0.5*i*gamma_star;
...
%         Acap(1,2)=Acap(1,2)-Acap(1+1,3)*Acap(1,1)/Acap(1+1,2);
        ...
        % P(z) such that P=(1/kh_jm)*dB/dz
        %  Note that this is evaluated at z=-zstar
        Pcap=zeros(size(Bcap));
        Pcap(1)=((Bcap(2)-Bcap(1))/dz)/kh_jm;
        for kz=2:nz-1
            Pcap(kz)=0.5*((Bcap(kz+1)-Bcap(kz-1))/dz)/kh_jm;
        end
        Pcap(nz)=((Bcap(nz)-Bcap(nz-1))/dz)/kh_jm;
...
for jm=1:jmax
    figure
    plot(real(Pcap_jm(:,1,jm)),zz)
    hold on
    plot(imag(Pcap_jm(:,1,jm)),zz)
    for momg=2:10:nomg
        plot(real(Pcap_jm(:,momg,jm)),zz)
        plot(imag(Pcap_jm(:,momg,jm)),zz)
    end
    plot(PcapWKB_jm(:,jm),zzb,'-g','LineWidth',3)
...
% for jm=1:jmax
%     figure
%     plot(real(Pcap_jm(:,1,jm))/abs(Pcap_jm(1,1,jm)),zz)
%     hold on
%     plot(imag(Pcap_jm(:,1,jm))/abs(Pcap_jm(1,1,jm)),zz)
%     for momg=2:10:nomg
%         plot(real(Pcap_jm(:,momg,jm))/abs(Pcap_jm(1,momg,jm)),zz)
%         plot(imag(Pcap_jm(:,momg,jm))/abs(Pcap_jm(1,momg,jm)),zz)
%     end
%     plot(PcapWKB_jm(:,jm),zzb,'-g','LineWidth',3)
%     xlabel('P_j/P_j(z=-z_*)','FontSize',fsa)
%     ylabel('z (m)','FontSize',fsa)
...
hold on
plot(imag(Pcap_jm(:,1,1)-Pcap_jm(1,1,1)),zz)
for momg=2:10:nomg
